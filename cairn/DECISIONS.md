...
### D-017 (2026-09-22): A model-management reply is checked for field types, and a load status other than "loaded" is a bad response (extends D-007 and D-016)

**Context:** M026 and M027 check the status-200 bodies of `list_models()`, `lms_load()`, `lms_download()`, and `lms_download_status()`. Before, a body with the wrong shape, such as `{}`, gave an empty data frame, `TRUE`, an unclassed error, or `rlmstudio_api_error` with status 200. D-007 rejected an API error with status 200, because the `status` field then tells a caller nothing. `lms_server_ready()` checked the model list with rules of its own.
**Decision:** Each function checks the fields that it reads for type, by exact name, before it reads them. A body that breaks a rule aborts with `rlmstudio_bad_response`. The rules check types and not status values, apart from `"loaded"`, the one load status the docs list. A load body with another status aborts with `rlmstudio_bad_response`. `lms_download()` no longer returns `TRUE`. `lms_server_ready()` applies the rules of `list_models()`. `lms_unload_all()` reads each instance by its `id` alone. Considered a check of every status value against the docs, rejected because a new status from a later LM Studio then fails every call. Considered `rlmstudio_api_error` for a load status other than `"loaded"`, rejected for D-007's reason. Considered the old `identifier` and first-column fallbacks, rejected because the docs name `id`. The pre-1.0 waiver (D-001) covers the lost `TRUE` return.
**Consequences:** `lms_server_ready()` returns `TRUE` for a model list only if `list_models()` can read it. A `"failed"` download status still passes, and a candidate row holds that question. A live LM Studio reply that breaks a rule falsifies the rule it breaks.
