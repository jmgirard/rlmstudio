...
Review pass 2, 2026-09-22. `origin/main` has not moved since the branch was cut, so no merge was needed. `devtools::test()` gave 4810 expectations, 0 failed, 0 skipped.

- AC1: PASS. The tests are in `tests/testthat/test-chat-batch.R`. The test "a native data-frame batch adds the reply id and stats of each reply" mocks two replies with a different value in every field. The token counts are JSON integers, and the timings have fractions. It asserts the column order and every cell with `expect_identical()` for both `logprobs` settings. With `logprobs = TRUE`, it expects two warnings, each the native logprobs warning, and with `logprobs = FALSE` none. The test "a native data frame keeps the row names that named inputs give" compares the row names to an OpenAI batch.
- AC2: PASS. The test "a field that is not one value of its column type gives NA" runs with `logprobs = FALSE`. It covers `response_id`, `input_tokens`, and `tokens_per_second`, each absent, `null`, the other scalar type, a boolean, an array, and an object. The array and object cases wrap a value of the right type. Each case asserts the `NA` cell, the kept `output`, and `expect_no_warning()`. The stats-shape test covers `stats` absent, `null`, an array, a string, a number, and `{}` with the same three assertions. A separate test keeps an empty `response_id` as `""`.
- AC3: PASS. The test "a failed input holds NA in every reply column" covers two failures. A 400 reply raises `rlmstudio_api_error`. An unreadable reply raises `rlmstudio_bad_response`, and it holds a valid `stats` object and `response_id`. Each failure runs with both `logprobs` settings. Each run asserts `NA` in the seven columns of the failed row and one failure warning at position 2. The test "the reply columns are there when every input failed" asserts the column order and types. It covers both classes and both settings.
- AC4: PASS. The test "the other routes add no reply column to a data frame" runs OpenResponses and OpenAI batches. It asserts `input` and `output`, and with `logprobs = TRUE` it asserts `input`, `output`, and `logprobs`. It also asserts the answer text and no warning. An OpenAI batch with a `schema` gives the columns `input` and `output`.
- AC5: PASS. The test "a single native call still returns one plain string" asserts one string with `NULL` attributes from `lms_chat_native()` and from `lms_chat(api_type = "native")`. With `simplify = FALSE`, it asserts `response_id` and two `stats` fields in the body. A vector batch has only the `names` attribute, and each list element has `NULL` attributes. A list batch stores `rlmstudio_api_error` with status 400 and `rlmstudio_bad_response`. A lost server in a native data frame aborts with `rlmstudio_no_server`, and `results` is `list("x", NULL, NULL)`.
