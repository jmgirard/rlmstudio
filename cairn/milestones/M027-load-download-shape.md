...
- 2026-09-22: step-7 approval: m027-load-download-shape approved for merge. The fix-now work (O5 tests, D4 wording) and the follow-up row land before the push. `devtools::test()` then gave 0 failures and 9099 passes.
...
- Triage at the step-7 gate. The user accepted the proposal.
  - Fix now: O5 and D4. `empty_body_fault()` adds a bare `{}` case to the fault test of each function. The case expects the message to name `status` for load and download, and `job_id` for status. On `main`'s code, the grown fault tests fail 46 of 46, 25 of 25, and 43 of 43. The merged ROADMAP row again says "even when the parse succeeds".
  - Follow-up: O2, O6, and O7 share one new ROADMAP row on the download flow. That row keeps the file at 59 lines only because the two M021 `logprobs` rows merged.
  - Reject: O1, because the JSON standard leaves duplicate keys undefined. O4, because it is M026 code and does no harm. O8, because it is style only. D5, because it is cosmetic.
  - Closed with no change: O3 and D6, by the AC6 amendment and pass 2. D1, D2, and D3, which are now in the record.
