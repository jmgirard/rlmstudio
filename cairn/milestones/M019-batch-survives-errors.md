...
Pass 2, 2026-09-22, on head `52db6b7`, with `main` already merged in. Full suite: 2156 expectations, 0 failed, 0 skipped. `test-chat-batch.R`: 14 tests, 316 expectations, 0 failed.

- AC1 pass 2: The same tests as pass 1 pass, and each asserts 3 requests.
- AC2 pass 2: "a batch in which every input fails keeps its logprobs column" asserts the columns `input`, `output`, and `logprobs` for both classes. It also asserts `NA` in every `output` row and `NULL` in every `logprobs` slot. It failed on the pass-1 code (T8). The pass-1 slot assertions still pass. "a reply with null content keeps its slot as NA in text results" asserts a vector and a data frame as long as `inputs`.
- AC3 pass 2: The pass-1 assertions pass, and the loop's warning count now names its case.
- AC4 pass 2: The pass-1 tests pass. A named-inputs test pins the names of `results`.
