...
- 2026-09-20 (M009): pak carries `mac.cran.dev` as a secondary source for macOS binaries, and that mirror lags CRAN. A 404 there makes pak save the error page as the `.tgz`, and `setup-r-dependencies` then dies with "unknown archive type" before the package is built. Check the mirror for the named file before treating a red macOS job as a package fault.
