...
_Last hygiene check: 2026-09-22 (M021 done and archived, M018 row pruned, one review candidate added)_
...
| M022 | A native chat batch reports each reply's stats and response id | in-progress | none | normal | milestones/M022-native-batch-stats.md |
...
- Token counts and the reply id in a data-frame batch on the OpenResponses and OpenAI routes. Their `usage` and `id` fields differ from the native `stats` block. M022 covers the native route only, added 2026-09-22, M022 plan gate
