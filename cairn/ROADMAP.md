...
- `lms_server_start()` returns before the REST API answers. A `lms_server_ready()` call right after it reports FALSE on a healthy machine, A vignette or script then skips work it can do. Give the check a bounded wait, or document the retry loop, added 2026-09-20, M010 T5
