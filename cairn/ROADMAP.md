...
| M024 | A chat reply that does not parse as JSON fails its input alone | in-progress | none | normal | milestones/M024-chat-body-parse.md |
...
- `lms_load()`, `lms_download()`, and `lms_download_status()` read a 200 body with `httr2::resp_body_json()` and no guard. `list_models()` reads it with `jsonlite::fromJSON()` and no guard (M024 implement). A body that is not JSON, or that comes as `text/plain`, fails with an unclassed error. M024 guards the chat routes and `lms_embed()` only, added 2026-09-22, M024 plan gate
