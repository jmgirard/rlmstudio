...
On a headless box the started server is the only thing you can see, so check
that it answers before you call it. `lms_server_ready()` asks the host for a
model list. It reports `FALSE` for a port held by another process, for a
server that is still coming up, and for a server that turned your token away.
