...
any of the three, for a reply that holds no readable answer text. With
either setting of `simplify`, it raises `rlmstudio_bad_response` through
any of the three for a status-200 body that does not parse as JSON.
...
and
[`lms_unload_all()`](https://jmgirard.github.io/rlmstudio/reference/lms_unload_all.md).
`lms_chat()` raises it through the chat function it calls.
[`lms_unload_all()`](https://jmgirard.github.io/rlmstudio/reference/lms_unload_all.md)
raises it through
[`list_models()`](https://jmgirard.github.io/rlmstudio/reference/list_models.md),
and so does
...
`choices` field instead. `lms_chat()` can raise the condition through
all three chat functions.
